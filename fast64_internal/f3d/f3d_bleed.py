...
class BleedGraphics:
    ...
    def inline_triGroup(
        ...
        tri_cmds = [c for c in tri_list.commands if type(c) == SP1Triangle or type(c) == SP2Triangles or type(c) == SPTriSnake or type(c) == SPContinueSnake]
        if tri_cmds:
    ...
    def bleed_between_tris(self, cmd_list: GfxList, cmd: GbiMacro, bleed_state: int, conflict_cmds: list[GbiMacro]):
        ...
        for parse_cmd in cmd_list.commands:
            if parse_cmd is cmd:
                return tri_buffered
            if type(parse_cmd) in [SP2Triangles, SP1Triangle, SPLine3D, SPLineW3D, SPTriSnake, SPContinueSnake]:
                tri_buffered = False
                continue
